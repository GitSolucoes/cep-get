...
<body>
  ...
  <main>
    <div style="text-align: center; margin-top: 10px">
      <label for="origem-select">Origem dos dados:</label>
      <select id="origem-select" class="form-campo" style="width: auto; display: inline-block;">
        <option value="deals">Deals</option>
        <option value="serasa">Serasa</option>
      </select>
    </div>

    <!-- [demais formulários continuam os mesmos] -->

    <div id="resultado"></div>
  </main>

<script>
  async function buscarGenerico(endpoint, valor, carregandoId) {
    ...
    const param = endpoint.split("-")[1] || "param";
    const response = await fetch(`${endpoint}?${param}=${encodeURIComponent(valor)}&origem=${origem}`);
    ...
    if (response.ok) {
      ...
      resultadoDiv.innerHTML = '<div class="nenhum-resultado">Erro na consulta.</div>';
    }
  }
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("rua-form")?.addEventListener("submit", (e) => {
      e.preventDefault();
      buscarGenerico("/buscar-rua", document.getElementById("rua").value, "carregando-rua");
    });
    document.getElementById("bairro-form")?.addEventListener("submit", (e) => {
      e.preventDefault();
      buscarGenerico("/buscar-bairro", document.getElementById("bairro").value, "carregando-bairro");
    });
    document.getElementById("cidade-form")?.addEventListener("submit", (e) => {
      e.preventDefault();
      buscarGenerico("/buscar-cidade", document.getElementById("cidade").value, "carregando-cidade");
    });
    document.getElementById("estado-form")?.addEventListener("submit", (e) => {
      e.preventDefault();
      buscarGenerico("/buscar-estado", document.getElementById("estado").value, "carregando-estado");
    });
